...
[OMEGA_A, OMEGA_B] = test_gradient(O);


function [OMEGA_A, OMEGA_B] = test_gradient(O)
...
% A is randomly initialized .
    ...
    % Iterations
    for j =1:100
        [~ , OMEGA_A ]= gradientLA_norm (A ,B ,O ,c ); % Compute gradient
        % ( normalized version )
        A=A + eta * OMEGA_A ; % Update matrix A
        [~ , lik]= forward_algorithm_norm (A ,B ,O ,c ); % Compute performance
        Likelihood (1 , j )= lik ;
    end

    %% Second , considering only update in matrix B . A and c are known and B is randomly initialized .
    A =[0.7 0.3;0.4 0.6];
    B= rand (2 ,3) ;
    ...
    % Iterations
    for j =1:100
        [~ , OMEGA_B ]= gradientLB_norm (A ,B ,O ,c ); % Compute gradient
        % ( normalized version )
        B=B + eta * OMEGA_B ; % Update matrix B
        [~ , lik ]= forward_algorithm_norm (A ,B ,O ,c ); % Compute performance
        Likelihood (2 , j )= lik ;
    end
